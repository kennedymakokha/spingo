...
const updatePassword = async (req, res) => {
    try {
        ...
        const salt = await bcryptjs_1.default.genSalt(10); // Generate a salt
        let phone = await (0, simplefunctions_1.Format_phone_number)(phone_number);
        const hashedPassword = await bcryptjs_1.default.hash(newPassword, salt); // Hash the new password
...
const requestToken = async (req, res) => {
    try {
        ...
        let textbody = { re: "otp request", id: user === null || user === void 0 ? void 0 : user._id, address: `${phone}`, Body: `Hi \nYour referal link is http://localhost:3000?affiliate=${1245}  ` };
