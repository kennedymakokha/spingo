...
export const place_bet = async (req: Request | any, res: Response | any) => {
    try {
        ...
        await Wallet.findOneAndUpdate({ user_id: req.user._id }, { total_amount: new_wallet_ammount }, { new: true, useFindAndModify: false })
        await new Contribution({ user_id: req.user._id, amount: req.body.stake, type }).save()
